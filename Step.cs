...
플레이어와 맵 사이의 x 축 거리를 계산 + 플레이어와 맵 사이의 y 축 거리를 계산
...
오브젝트 풀링(Object Pooling)을 구현
오브젝트 풀링은 게임에서 자주 사용되는 게임 오브젝트를 미리 생성하여 재활용하는 방식으로, 
게임 실행 중에 오브젝트를 동적으로 생성하거나 삭제하는 대신 오브젝트를 미리 생성해놓고 필요할 때마다 
재활용하여 성능을 향상시키는 방법
*/
...
직렬화(Serialization)는 데이터나 객체를 일렬로 나열하여 저장하거나 전송하는 과정을 말한다. 
프로그래밍에서는 객체를 파일로 저장하거나 네트워크를 통해 전송하기 위해 객체를 바이트 단위로 변환하는 과정

데이터 저장: 객체를 파일이나 데이터베이스에 저장할 때 사용 
    저장된 데이터는 나중에 다시 읽어와서 객체로 복원
네트워크 통신: 객체를 네트워크를 통해 전송할 때 사용 
    데이터를 직렬화하여 전송하고, 수신측에서는 데이터를 역직렬화하여 객체로 복원
상태 저장 및 불러오기: 게임 등에서 현재 상태를 저장하고 나중에 불러올 때 사용

C#에서는 직렬화를 구현하기 위해 [System.Serializable] 어트리뷰트(Attribute)를 사용
    이 어트리뷰트를 클래스 앞에 붙여서 해당 클래스가 직렬화 가능하다고 표시한다.
*/
...
ScriptableObject는 유니티에서 스크립트 기반 데이터를 저장하고 관리하기 위한 특별한 유형의 객체다. 
MonoBehaviour를 상속하지 않으며, 게임 오브젝트와 컴포넌트의 인스턴스가 아니라 
스크립트로 정의된 데이터만 포함한다.
이러한 특성 때문에 게임 개발에서 데이터를 유연하게 관리하고 공유하는 데 매우 유용하다.

데이터 컨테이너: ScriptableObject는 데이터를 저장하기 위한 컨테이너로 사용된다. 
    논리적인 데이터 단위를 정의하고 해당 데이터를 저장하는 용도로 사용할 수 있다.

에디터에서 수정 가능: ScriptableObject는 에디터에서 생성하고 수정할 수 있으며, 
    수정된 데이터는 프로젝트에 저장된다. 
    이를 통해 에디터에서 게임 데이터를 생성, 관리 및 수정할 수 있습니다.

인스턴스화 가능: ScriptableObject는 스크립트로 생성된 인스턴스처럼 동작하며, 
    여러 곳에서 참조되어 공유할 수 있다. 
    동일한 데이터를 여러 오브젝트에서 사용할 수 있다.

리소스 폴더에 저장: ScriptableObject는 프로젝트의 "Resources" 폴더에 저장되어 리소스로 취급된다. 
    이를 통해 게임 실행 중에 리소스를 동적으로 로드하거나 전체 데이터 세트를 불러올 수 있다.

직렬화: ScriptableObject는 Unity의 직렬화 시스템을 활용하여 데이터를 저장하고 로드한다. 
    이를 통해 데이터를 유니티 프로젝트 파일에 저장하고 편집할 수 있다.

ScriptableObject 만들기 단계
    스크립트 생성: ScriptableObject를 표현하는 스크립트 클래스를 생성
        이 스크립트는 MonoBehaviour가 아닌 ScriptableObject를 상속
    데이터 정의: 스크립트 클래스 내에서 저장할 데이터 필드를 정의 
        이 데이터는 인스펙터 창에서 수정 가능
    에디터에서 생성: 에디터에서 스크립트를 인스턴스화하여 데이터를 설정하고 저장
        이 때 생성된 인스턴스는 프로젝트 리소스로 저장됨
    인스턴스 참조: 인스턴스화된 ScriptableObject는 스크립트에서 참조하여 데이터를 사용

ItemData -> Item --> Weapon --> Bullet
                ㄴ-> Gear

1. 타격의 주체가 될 오브젝트를 구성한다.
2. 무기를 플레이어에게 쥐어주어 무기의 행동 양식을 지정한다.
3. 앞으로 추가될 다양한 아이템을 관리하기 위해 스크립트블 오브젝트를 구축한다.
4. 아이템별 속성을 지정해 주고 Item 클래스를 만들어서 아이템을 구분짓는다.
*/
...
1. 먼저 플레이어의 자식으로 오브젝트를 만든다.
2. 정 방향으로 에디터 상에서 위치를 잡아 준다.
3. 스프라이트의 방향 전환을 담당할 클래스를 만든다.
4. 정 방향과 역 방향의 좌표와 회전값을 속성으로 초기화 한다.
5. 플레이어의 방향에 따라서 스프라이트 + 좌표 속성을 바꿔준다.
*/
